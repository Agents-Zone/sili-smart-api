...
// usageCacheTestSeq 为用例键提供进程内唯一序号：UnixNano 在 Windows 粗时钟粒度下
// 同 tick 内会重复，导致用例间 entry key 冲突、计数互相累加。
var usageCacheTestSeq atomic.Int64

func uniqueUsageCacheTestKey(prefix string) string {
	return fmt.Sprintf("%s_%d_%d", prefix, usageCacheTestSeq.Add(1), time.Now().UnixNano())
}
...
func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	ruleName := uniqueUsageCacheTestKey("rule")
	usingGroup := "default"
	keyFP := uniqueUsageCacheTestKey("fp")
