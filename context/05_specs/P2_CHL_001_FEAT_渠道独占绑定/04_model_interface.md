...
- 清除时机：管理员清空缓存（全部/按规则）、独占开关切换保存、请求终态失败回滚、渠道不可用清缓存（keep_on_channel_disabled=false 语义）、渠道路由配置被下游操作变更（SSOT 5.2.2 第7条）时，与正向缓存同批清除（SSOT 5.1.2 第4条、5.2.2 第4条）
...
清空操作（管理员清空、按规则清空、开关切换、终态失败回滚、渠道不可用清除、渠道配置变更清理）必须对正向缓存、反向占用索引、最近绑定记录三批同批执行。清理失败的重试与降级路径见接口文档第 5 节，残留条目随 TTL 自然过期兜底（SSOT 5.1.5）。
...
| v1.8 | 2026-09-11 | 监理扫描修复（模式二，补齐渠道批量编辑 P2_CHL_002 的依赖能力）：4.1 清除时机与 6.2 同批清理一致性补入"渠道配置变更清理"场景（对齐 SSOT v1.11 5.2.2 第7条）；文档信息表与文末版本号统一为 v1.8（原表内 v1.0、文末 v1.3、变更记录至 v1.7 三处不一致） |
