...
| 2 | 查询亲和缓存统计（含独占指标） | GET | `/api/option/channel_affinity_cache` | 存量接口，响应体扩展 3 个字段 | SSOT 4.2（F-统计弹窗）、5.3.3 |
...
| value | string | 是 | JSON 数组字符串，逐元素反序列化为规则结构；解析失败经现有 option 错误路径返回 | 规则数组序列化结果 |
...
**响应示例（失败，JSON 解析失败沿用现有错误路径）：**

```json
{
  "success": false,
  "message": "无效的 rules JSON"
}
```
...
2. 对发生变化的规则，清除该规则作用域内的正向亲和缓存条目（键前缀含规则名时按前缀清除；规则未启用 include_rule_name 时按旧规则定义的键构成回放清除）、反向占用索引条目、最近绑定记录条目
...
| 场景 | HTTP 状态 | 响应 |
|------|----------|------|
| value JSON 解析失败 | 200（业务错误路径） | `success:false` + message |
...
**需求追溯：** SSOT 4.2（统计弹窗新增三个只读指标）、4.2.4 规则1（指标口径）、5.3.2 第3条
...
**统计口径实现说明（SSOT 4.2.4 规则1）：** 遍历反向占用索引全部渠道条目，渠道条目键数为 1 计入独占、大于 1 计入复用；`exclusive_bindings` 与 `shared_bindings` 之和应等于正向绑定中登记反向索引的键总数（正向缓存中可能存在极少量索引登记失败的条目，两口径允许存在短暂偏差，以索引为准展示）。索引条目为渠道级整体 TTL，正向绑定已过期但条目未整体过期时成员残留，口径偏高最多一个 TTL 周期，属设计内滞后（详见模型文档 4.1）。
...
**需求追溯：** 现状接口，本 Feature 无变更。SSOT 4.2 的统计弹窗（CacheStatsDialog）现状为按亲和键的上游缓存命中详情查询，本功能在该弹窗新增的三个全局指标经接口 3.2 返回，此接口保持原样。

**说明：** 列出仅为接口清单完整性，请求参数（rule_name、using_group、key_hint、key_fp）、响应结构与现状一致，设计不涉及。
...
| 函数 | 签名 | 职责 | 需求追溯 |
|------|------|------|---------|
| 独占绑定决策 | `GetPreferredChannelByAffinity` 扩展：亲和 miss 且规则启用 exclusive_bind 时，进入独占选路；返回值扩展携带绑定模式（独占/复用） | 依据反向占用索引筛空闲渠道、满载降级最少绑定数优先、最近绑定记录优先绑回 | SSOT 5.1.2 第2条 |
...
索引内仅存亲和键指纹（沿用 `affinityFingerprint`，SHA1 前 8 位），不存原始键值（SSOT 5.2.3）。
...
| 敏感数据 | 反向占用索引与审计标记仅存亲和键指纹（SHA1 截断 8 位），不扩散原始键值 [需求：SSOT 5.2.3、8.1 键指纹] |
| 输入校验 | exclusive_bind 布尔类型经 JSON 反序列化天然约束；rules JSON 解析失败走现有错误路径 |
...
| v1.1 | 2026-09-01 | 监理扫描修复：3.2 统计口径补充索引成员级滞后窗口说明；3.4 参数清单补 key_hint |
