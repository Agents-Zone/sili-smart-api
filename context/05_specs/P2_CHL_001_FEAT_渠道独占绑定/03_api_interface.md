...
2. 对发生变化的规则，清除该规则作用域内的正向亲和缓存条目（键前缀含规则名时按前缀清除；规则未启用 include_rule_name 时按旧规则定义的键构成回放清除）、反向占用索引条目、最近绑定记录条目。工程边界：旧规则键构成（IncludeModelName/IncludeUsingGroup 组合 + 亲和键值）无法回放出可区分前缀时，退化为正向缓存与反向占用索引、最近绑定记录整体清空，并记 SysLog 声明该退化（清除范围扩大属工程兜底，保证旧语义数据不残留）
