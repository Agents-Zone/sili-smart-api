...
**按渠道清理（供下游调用，SSOT 5.2.2 第7条）：** 渠道的路由相关配置（models/group/weight/priority/tag）被下游操作（渠道批量编辑 P2_CHL_002 等）变更后，下游在自身事务提交后调用本模块导出的按渠道清理能力，对受影响渠道同批清除反向索引条目、最近绑定记录与该渠道仍持有的正向绑定（对齐 SSOT 5.2.4 规则2 的三批同清口径，不允许只清反向索引）；清理失败重试一次，仍失败记 SysError，残留随 TTL 收敛（SSOT 5.2.5），不阻断调用方结果。该能力经 service 层导出函数提供，无 HTTP 暴露。
...
| v1.9 | 2026-09-11 | 监理扫描修复（模式二，补齐渠道批量编辑 P2_CHL_002 的依赖能力）：4.2 新增"按渠道清理"内部契约（下游操作变更渠道路由配置后按渠道三批同清，对齐 SSOT 5.2.2 第7条与 5.2.4 规则2；经 service 层导出函数提供，无 HTTP 暴露） |
