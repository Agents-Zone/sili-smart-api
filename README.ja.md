...
チャネル排他バインドはランタイムの共有キャッシュに依存します。Redis を有効にせず（メモリキャッシュモード）複数インスタンスでデプロイした場合、逆方向の占有インデックスとアトミックなプレースホルダはインスタンス間で共有されず、排他判定と先着順のセマンティクスは既存のソフトアフィニティに退化します。インスタンス間での排他が必要な場合は Redis（`REDIS_CONN_STRING`）を設定してください。
